...
for (ii,order) in enumerate(orders):
    for (jj,trainSampleSize) in enumerate(trainSampleSizes):
        for (kk,interaction) in enumerate(interactions):
            print(f'Starting Order" {order} Samples {trainSampleSize} Interaction {interaction}')
            ...
            train_points,train_values = lennardJonesSamplesMod(order,trainSampleSize,c,exp)
            ...
            test_points,test_values = lennardJonesSamplesMod(order,testSampleSize,c,exp)
            ...
            np.save(f'exp_2_lennardjones.data',res)