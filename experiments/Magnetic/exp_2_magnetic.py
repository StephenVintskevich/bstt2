...
maxSweeps=7
res = np.zeros((len(order),len(interaction),len(trainSampleSize)))
b = np.pi
...
for ii in range(len(order)):
    for jj in range(len(trainSampleSize)):
        for kk in range(len(interaction)):
            ...
            coeffs = random_homogenous_polynomial_sum_system2([degree]*order[ii],degree,maxGroupSize,interaction[kk],S)
...
np.save(folder+'data/exp_2_magnetic.data',res)