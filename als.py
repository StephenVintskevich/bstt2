...
import time
damp_coef = 1e-10
print("als.py Last Changes: 19 Dec 1.24 PM")
...
#                 'ler, ne, nr -> nl', self.bstt.components[self.bstt.corePosition+1], self.measurements[self.bstt.corePosition+1], self.rightStack[-1],optimize=True))
#             self.rightH1GramianStack.append(np.einsum(
#                 'ijk, lmn, jm,kn -> il', self.bstt.components[self.bstt.corePosition+1],  self.bstt.components[self.bstt.corePosition+1], self.localH1Gramians[self.bstt.corePosition+1], self.rightH1GramianStack[-1],optimize= True))
#             self.rightL2GramianStack.append(np.einsum(
#                 'ijk, lmn, jm,kn -> il', self.bstt.components[self.bstt.corePosition+1],  self.bstt.components[self.bstt.corePosition+1], self.localL2Gramians[self.bstt.corePosition+1], self.rightL2GramianStack[-1],optimize= True))
#             if self.verbosity >= 2:
#                 if valid_stacks:
#                     print(
...
#                         assert np.allclose(np.einsum('ijk,ijl->kl', self.bstt.components[self.bstt.corePosition-1], self.bstt.components[self.bstt.corePosition-1],optimize= True), np.eye(
...
#                 'nl, ne, ler -> nr', self.leftStack[-1], self.measurements[self.bstt.corePosition-1], self.bstt.components[self.bstt.corePosition-1],optimize= True))
#             self.leftH1GramianStack.append(np.einsum(
#                 'ijk, lmn, jm,il -> kn', self.bstt.components[self.bstt.corePosition-1],  self.bstt.components[self.bstt.corePosition-1], self.localH1Gramians[self.bstt.corePosition-1], self.leftH1GramianStack[-1],optimize= True))
#             self.leftL2GramianStack.append(np.einsum(
#                 'ijk, lmn, jm,il -> kn', self.bstt.components[self.bstt.corePosition-1],  self.bstt.components[self.bstt.corePosition-1], self.localL2Gramians[self.bstt.corePosition-1], self.leftL2GramianStack[-1],optimize= True))
#             if self.verbosity >= 2:
#                 if valid_stacks:
#                     print(
#                         f"move_core {self.bstt.corePosition-1} --> {self.bstt.corePosition}.  (residual: {pre_res:.2e} --> {self.residual():.2e})")
...
#         R = self.rightStack[-1]
#         pred = np.einsum('ler,nl,ne,nr -> n', core, L, E, R,optimize= True)
#         return np.linalg.norm(pred - self.values) / np.linalg.norm(self.values)

#     def calculate_update(self, slc, _direction):
#         if _direction == 'left':
#             Gramian = np.einsum(
#                 'ij,kl->ikjl', self.leftH1GramianStack[-1], self.localH1Gramians[self.bstt.corePosition-1],optimize= True)
...
#             projGramian = np.einsum('ijkl,ijm,kln->mn', Gramian, ns, ns,optimize= True)
#             pGe, pGP = np.linalg.eigh(projGramian)
#             return np.einsum('ijk,k->ij', ns, pGP[0],optimize= True)
...
#                               L[:, block[0]], E[:, block[1]], R[:, block[2]],optimize= True)
...
#                 tr = np.einsum('il,jm,kn->ijklmn', LP, EP, RP,optimize= True)
#                 Tr_blocks.append(tr.reshape(
#                     Op_blocks[-1].shape[1], Op_blocks[-1].shape[1]))
    
#                 Weights.extend(np.einsum('i,j,k->ijk', Le, Ee, Re).reshape(-1),optimize= True)
...
#             Op_blocks = []
#             for block in coreBlocks:
#                 op = np.einsum('nl,ne,nr -> nler', L[:, block[0]], E[:, block[1]], R[:, block[2]],optimize= True)
#                 Op_blocks.append(op.reshape(N,-1))
#             Op = np.concatenate(Op_blocks, axis=1)
#             # Res = np.linalg.solve(Op.T @ Op, Op.T @ self.values)
#             Res = lsqr(Op, self.values, damp=damp_coef)[0]
...
#             comp_measure = np.einsum('ler, me  -> lmr', self.bstt.components[self.bstt.corePosition+1], self.measurements[self.bstt.corePosition+1],optimize= True)
#             if self.bstt.corePosition+1 == self.bstt.order-1:
#                 self.rightStack1.append(np.einsum('imk, lmn, kmn -> iml', comp_measure,comp_measure, self.rightStack1[-1],optimize= True))
#                 self.rightStack2.append(np.einsum('imk, lmn, kmn -> iml', comp_measure,comp_measure, self.rightStack2[-1],optimize= True))
#                 self.rightStack1rhs.append(np.einsum('imk, km -> im', comp_measure, self.rightStack1rhs[-1],optimize= True))
#                 self.rightStack2rhs.append(np.einsum('imk, km -> im', comp_measure, self.rightStack2rhs[-1],optimize= True))
#             else:
#                 comp_measure_grad = np.einsum('ler, me  -> lmr', self.bstt.components[self.bstt.corePosition+1], self.measurements_grad[self.bstt.corePosition+1],optimize= True)
#                 stack2 = np.einsum('imk, lmn, kmn -> iml', comp_measure_grad,comp_measure_grad, self.rightStack1[-1],optimize= True)
#                 stack2rhs = np.einsum('imk, m,km -> im', comp_measure_grad,self.values[:,self.bstt.corePosition+1],  self.rightStack1rhs[-1],optimize= True)
#                 if self.bstt.corePosition+1 < self.bstt.order-2:
#                     stack2 += np.einsum('imk, lmn, kmn -> iml', comp_measure,comp_measure, self.rightStack2[-1],optimize= True)
#                     stack2rhs += np.einsum('imk, km -> im', comp_measure, self.rightStack2rhs[-1],optimize= True)
...
#             comp_measure = np.einsum('ler, me  -> lmr', self.bstt.components[self.bstt.corePosition-1], self.measurements[self.bstt.corePosition-1],optimize= True)
#             comp_measure_grad = np.einsum('ler, me  -> lmr', self.bstt.components[self.bstt.corePosition-1], self.measurements_grad[self.bstt.corePosition-1],optimize= True)
#             stack2 = np.einsum('iml,imk, lmn -> kmn',  self.leftStack1[-1], comp_measure_grad, comp_measure_grad,optimize= True)
#             stack2rhs = np.einsum('im, m,imk -> km', self.leftStack1rhs[-1],self.values[:,self.bstt.corePosition-1], comp_measure_grad,optimize= True)
#             if self.bstt.corePosition-1 > 0:
#                 stack2 += np.einsum('iml,imk, lmn -> kmn',   self.leftStack2[-1], comp_measure, comp_measure,optimize= True)
#                 stack2rhs += np.einsum('im, imk -> km', self.leftStack2rhs[-1],comp_measure,optimize= True)

#             self.leftStack2.append(stack2)
#             self.leftStack1.append(np.einsum('iml,imk, lmn -> kmn',  self.leftStack1[-1],comp_measure,comp_measure,optimize= True))
            
#             self.leftStack2rhs.append(stack2rhs)
#             self.leftStack1rhs.append(np.einsum('im, imk -> km', self.leftStack1rhs[-1],comp_measure,optimize= True))
#             if self.verbosity >= 2:
#                 if valid_stacks:
#                     print(f"move_core {self.bstt.corePosition-1} --> {self.bstt.corePosition}.  (residual: {pre_res:.2e} --> {self.residual():.2e})")
#                 else:
...
#         E_op = np.einsum('mp,mq->pmq',E,E,optimize= True)
#         E_grad_op = np.einsum('mp,mq->pmq',E_grad,E_grad,optimize= True)
...
#                 op = np.einsum('imk,pmq,lmn -> iplkqn', L2[block1[0],:, block2[0]], E_op[block1[1],:, block2[1]], R1[block1[2],:, block2[2]],optimize= True)
#                 if self.bstt.corePosition < self.bstt.order-2:
#                     op += np.einsum('imk,pmq,lmn -> iplkqn', L1[block1[0],:, block2[0]], E_op[block1[1],:, block2[1]], R2[block1[2],:, block2[2]],optimize= True)
#                 op += np.einsum('imk,pmq,lmn -> iplkqn', L1[block1[0],:, block2[0]], E_grad_op[block1[1],:, block2[1]], R1[block1[2],:, block2[2]],optimize= True)
#                 dim1 = op.shape[0]*op.shape[1]*op.shape[2]
#                 dim2 = op.shape[3]*op.shape[4]*op.shape[5]
#                 Op_blocks_col.append( op.reshape(dim1,dim2))
            
#             rhs = np.einsum('im,mp,lm -> ipl', L2rhs[block1[0],:], E[:,block1[1]], R1rhs[block1[2],:],optimize= True)
#             if self.bstt.corePosition < self.bstt.order-2:
#                 rhs += np.einsum('im,mp,lm -> ipl', L1rhs[block1[0],:], E[:,block1[1]], R2rhs[block1[2],:],optimize= True)
#             rhs += np.einsum('im,mp,lm,m -> ipl', L1rhs[block1[0],:], E_grad[:,block1[1]], R1rhs[block1[2],:],self.values[:,self.bstt.corePosition],optimize= True)
...
#         Res = lsqr(Op, Rhs, damp = damp_coef)[0]
#         #Res, *_ = np.linalg.lstsq(Op, self.values, rcond=None)  # When Op.T@Op is singular (less samples then dofs in this component) then lstsq returns the minimal norm solution.
#         core[...] = BlockSparseTensor(Res, coreBlocks, core.shape).toarray()
...
#                                   self.measurements[self.bstt.corePosition+1], Smat, self.rightStack[-1],optimize= True))
...
#             # self.rightL2GramianStack.pop()
#             self.leftStack.append(np.einsum(
#                 'nld, ne, md, lemr -> nrd', self.leftStack[-1], self.measurements[self.bstt.corePosition-1], Smat, comp,optimize= True))
...
#         pred = np.einsum('lesr,nld,ne,sd,nrd -> nd', core, L, E, S, R,optimize= True)
...
#                           L[:, block[0], :], E[:, block[1]], S[block[2], :], R[:, block[3], :],optimize= True)
#             Op_blocks.append(op)
...
class ALSSystem2(object):
        ...
        if self.direction == 'left':
            ...
            for eq in range(self.coeffs.numberOfEquations):
                ...
                newStack.append(np.einsum('ler, me, mr -> ml', comp,
                                self.measurements[self.coeffs.corePosition+1], self.rightStack[-1][eq],optimize= True))
            ...
            for eq in range(self.coeffs.numberOfEquations):
                ...
                newStack.append(np.einsum(
                    'ml, me, ler -> mr', self.leftStack[-1][eq], self.measurements[self.coeffs.corePosition-1], comp,optimize= True))
    ...
    def residual(self):
        ...
        for eq in range(self.coeffs.numberOfEquations):
            ...
            pred.append(np.einsum('ler,ml,me,mr -> m', core, L, E, R,optimize= True))
    ...
    def microstep(self):
        ...
        for eq in range(self.coeffs.numberOfEquations):
            ...
            for block in coreBlocks:
                op = np.einsum(
                    'ml,me,mr -> mler', L[eq][:, block[0]], E[:, block[1]], R[eq][:, block[2]],optimize= True)
            ...
            if sum(eqs) == 1 or (self.direction == 'right' and k == self.coeffs.interactions-1 and self.coeffs.corePosition > 0) or  (self.direction == 'left' and k == 0 and self.coeffs.corePosition < self.coeffs.order-1):
                ...
                for switched_eq in switched_eqs:
                    R_new = np.einsum('ler, me, mr -> ml', core,
                                    self.measurements[self.coeffs.corePosition], R[switched_eq],optimize=True)
                    Op_blocks_switched_eq = []
                    for block in blocks_switched_eq:
                        op = np.einsum(
                            'ml,mr -> mlr', L[switched_eq][:, block[0]], R_new[:, block[1]],optimize=True)
                    ...
                    #Res_switched_eq = lsqr(Op_switched_eq, rhs_switched_eq, damp = damp_coef)[0]
                    ...
                    self.leftStack[-1][switched_eq] = np.einsum(
                       'ml, lr -> mr', self.leftStack[-1][switched_eq], core_switched_eq,optimize= True)
                    comp =  self.coeffs.bstts[ self.coeffs.selectionMatrix[switched_eq, 
                        self.coeffs.corePosition-1]].components[self.coeffs.corePosition-1]
                    self.coeffs.bstts[ self.coeffs.selectionMatrix[switched_eq, 
                        self.coeffs.corePosition-1]].components[self.coeffs.corePosition-1] = \
                        np.einsum('ler,rs->les',comp,core_switched_eq,optimize= True)
                ...
                rhs = self.values[:, eqs2].reshape(-1, order='F')
                ### Scipy standard toolbox regularizer
                #Res = lsqr(Op, rhs, damp = damp_coef)[0]
                Res, *_ = np.linalg.lstsq(Op, rhs, rcond=None)
                #Res = np.linalg.solve(Op.T@Op+self.alpha*np.eye(Op.shape[1]), Op.T@rhs)
                core[:, :, :] = BlockSparseTensor(
                ...
                for switched_eq in switched_eqs:
                    ...
                    L_new = np.einsum( 'ml, me, ler -> mr', L[switched_eq], 
                                      self.measurements[self.coeffs.corePosition], core,optimize= True)
                    Op_blocks_switched_eq = []
                    for block in blocks_switched_eq:
                        op = np.einsum(
                            'ml,mr -> mlr', L_new[:, block[0]], R[switched_eq][:, block[1]],optimize= True)
                        Op_blocks_switched_eq.append(op.reshape(self.numberOfSamples, -1))
                    Op_switched_eq = np.concatenate(Op_blocks_switched_eq, axis=1)
                    rhs_switched_eq = self.values[:, switched_eq].reshape(-1, order='F')
                    ### Scipy standard toolbox regularizer
                    ...
                    self.rightStack[-1][switched_eq] = np.einsum(
                       'lr, mr -> ml', core_switched_eq,self.rightStack[-1][switched_eq],optimize= True)
                    comp =  self.coeffs.bstts[ self.coeffs.selectionMatrix[switched_eq, 
                        self.coeffs.corePosition+1]].components[self.coeffs.corePosition+1]
                    self.coeffs.bstts[ self.coeffs.selectionMatrix[switched_eq, 
                        self.coeffs.corePosition+1]].components[self.coeffs.corePosition+1] = \
                        np.einsum('kl,ler->ker',core_switched_eq,comp,optimize= True)
